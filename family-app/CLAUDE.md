...
Production is `https://family-hub-amber.vercel.app`, served by the Vercel project `family-hub` (this directory is linked via `.vercel/project.json`). The project's Root Directory is `family-app`, and the GitHub integration is connected — `git push origin main` auto-deploys.

Standard flow after editing code:
1. Commit and `git push origin main`. Vercel builds and aliases automatically (~30–90s).
2. Verify the deploy is live by comparing asset hashes:
   ```bash
   npm run build  # note the dist/assets/index-XXXX.css hash
   curl -s https://family-hub-amber.vercel.app/ | grep -oE '/assets/index-[^"]+\.css'
   ```
   The deployed hash must match the local one. You can also confirm Vercel's build status:
   ```bash
   gh api repos/travisgmader/hello-pioneer/commits/<sha>/status
   ```
3. If the auto-deploy fails or stalls, deploy manually from this directory: `vercel --prod --yes`.

If the deployed hash doesn't match after a reasonable wait, investigate (Vercel project link, build error) — don't shrug it off as "cache."
