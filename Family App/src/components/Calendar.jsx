...
export default function Calendar({
  ...
  const eventsForDate = (dateStr) => {
    ...
    return [...evs, ...choreEvs].sort((a, b) => (a.time || '').localeCompare(b.time || ''));
  };
  ...
  const evDisplayColor = (ev) => COLOR_VARS[ev.color] || COLOR_VARS.lavender;
  ...
  for (let i = firstDay - 1; i >= 0; i--) {
    ...
    cells.push({ y, m, d, isOther: true });
  }
  for (let d = 1; d <= daysInMonth; d++) {
    cells.push({ y: year, m: month, d, isOther: false });
  }
  let nextDay = 1;
  while (cells.length % 7 !== 0) {
    const m = month === 11 ? 0 : month + 1;
    const y = month === 11 ? year + 1 : year;
    cells.push({ y, m, d: nextDay++, isOther: true });
  }
      ...
      <div className={styles.gridScroll}><div className={styles.grid}>
        ...
        {cells.map((cell, i) => {
          ...
          return (
            <div
              key={`${y}-${m}-${d}-${i}`}
              className={`${styles.cell} ${isToday ? styles.today : ''} ${isSel ? styles.selected : ''} ${isOther ? styles.otherMonth : ''}`}
            ...
            >
              ...
              <div className={styles.evtChips}>
                {evs.slice(0, 3).map(ev => {
                  ...
                  const dropCol = ev.dropoffParent ? parentColor(ev.dropoffParent) : null;
                  const pickCol = ev.pickupParent  ? parentColor(ev.pickupParent)  : null;
                  const chipStyle = {
                    background: col.bg,
                    color: col.dark,
        ...
        <div className={styles.overlay} onClick={closeDialog}>
          ...
          <div className={styles.detailMeals}>
            {MEAL_SLOTS_ORDER.map(slot => {
              ...
              const ms = MEAL_SLOT_STYLES[slot];
              const isEditingMeal = editingMealSlot === slot;
              return (
                <div key={slot} className={styles.detailMealRow} style={{ background: ms.bg, borderLeftColor: ms.accent }}>
                  ...
                  {isEditingMeal ? (
                    <div className={styles.mealEditWrap}>
                      <input
                        autoFocus
                        className={styles.mealEditInput}
                        value={editMealValue}
                        onChange={e => setEditMealValue(e.target.value)}
                        placeholder="Meal name..."
                        onKeyDown={e => {
                          if (e.key === 'Enter') { onSetMeal?.(selectedDateStr, slot, editMealValue.trim()); setEditingMealSlot(null); }
                          if (e.key === 'Escape') setEditingMealSlot(null);
                        }}
                      />
                      <button className={styles.mealEditSave} style={{ background: ms.accent }} onClick={() => { onSetMeal?.(selectedDateStr, slot, editMealValue.trim()); setEditingMealSlot(null); }}>✓</button>
                      <button className={styles.mealEditCancel} onClick={() => setEditingMealSlot(null)}>✕</button>
                    </div>
                  ) : (
                    <button
                      className={styles.mealEditBtn}
                      style={{ color: meal ? 'var(--text)' : 'var(--text-muted)' }}
                      onClick={onSetMeal ? () => { setEditingMealSlot(slot); setEditMealValue(meal); } : undefined}
                      disabled={!onSetMeal}
                    >
                      {meal || 'Not planned'}
                      {onSetMeal && <span className={styles.mealEditIcon}>✎</span>}
                    </button>
                  )}
                </div>
              );
          ...
          {selectedEvents.length === 0 ? (
            ...
            <div className={styles.evtList}>
              {selectedEvents.map(ev => {
                ...
                const pickCol = ev.pickupParent  ? parentColor(ev.pickupParent)  : null;
                const itemStyle = {
                  borderLeftColor: dropCol ? dropCol.accent : col.accent,
                  borderLeftWidth: dropCol ? '5px' : undefined,
                  borderRight: pickCol ? `5px solid ${pickCol.accent}` : undefined,
                  background: col.bg,
                };
                  ...
                  >
                    {isEditing ? (
                      ...
                      <>
                        ...
                        {/* Dropoff / Pickup selectors for all events */}
                        {!ev.isChore && onUpdateEvent && (
                          <>
                            {[
                              { key: 'dropoffParent', label: '⬇️ Dropoff' },
                              { key: 'pickupParent',  label: '⬆️ Pickup' },
                            ].map(({ key, label }) => (
                              <div key={key} className={styles.transportRow}>
                                <span className={styles.transportLabel}>{label}:</span>
                                {PARENTS.map(p => {
                                  const isActive = ev[key] === p.id;
                                  ...
                                  return (
                                    <button
                                      ...
                                      onClick={() => onUpdateEvent(ev.id, { [key]: isActive ? null : p.id })}
                                    >
                                      {p.emoji} {p.label}
                                    </button>
                                  );
